...
lv_font_t * lv_font_load(const char * font_name)
{
    ...
    lv_font_t * font = heap_caps_malloc(sizeof(lv_font_t), MALLOC_CAP_SPIRAM);
    if(font) {
            ...
            if(dsc->kern_classes == 0) {
                ...
                if(NULL != kern_dsc) {
                    if(kern_dsc->glyph_ids)
                        free((void *)kern_dsc->glyph_ids);

                    if(kern_dsc->values)
                        free((void *)kern_dsc->values);
                ...
                if(NULL != kern_dsc) {
                    ...
                    if(kern_dsc->left_class_mapping)
                        free((void *)kern_dsc->left_class_mapping);
            ...
            if(NULL != cmaps) {
                for(int i = 0; i < dsc->cmap_num; ++i) {
                    if(NULL != cmaps[i].glyph_id_ofs_list)
                        free((void *)cmaps[i].glyph_id_ofs_list);
                    if(NULL != cmaps[i].unicode_list)
                        free((void *)cmaps[i].unicode_list);
                }
                free(cmaps);
            }
    ...
    for(unsigned int i = 0; i < font_dsc->cmap_num; ++i) {
        ...
        switch(cmap_table[i].format_type) {
            case LV_FONT_FMT_TXT_CMAP_FORMAT0_FULL: {
                    ...
                    uint8_t * glyph_id_ofs_list = heap_caps_malloc(ids_size, MALLOC_CAP_SPIRAM);
            ...
            case LV_FONT_FMT_TXT_CMAP_SPARSE_TINY: {
                    ...
                    uint16_t * unicode_list = (uint16_t *)heap_caps_malloc(list_size, MALLOC_CAP_SPIRAM);
                    ...
                    if(cmap_table[i].format_type == LV_FONT_FMT_TXT_CMAP_SPARSE_FULL) {
                        uint16_t * buf = heap_caps_malloc(sizeof(uint16_t) * cmap->list_length, MALLOC_CAP_SPIRAM);
...
static int32_t load_cmaps(lv_fs_file_t * fp, lv_font_fmt_txt_dsc_t * font_dsc, uint32_t cmaps_start)
{
    ...
    lv_font_fmt_txt_cmap_t * cmaps =
        heap_caps_malloc(cmaps_subtables_count * sizeof(lv_font_fmt_txt_cmap_t), MALLOC_CAP_SPIRAM);
    ...
    cmap_table_bin_t * cmaps_tables = heap_caps_malloc(sizeof(cmap_table_bin_t) * font_dsc->cmap_num, MALLOC_CAP_SPIRAM);
                          ...
                          uint32_t start, uint32_t * glyph_offset, uint32_t loca_count, font_header_bin_t * header)
{
    ...
    lv_font_fmt_txt_glyph_dsc_t * glyph_dsc = (lv_font_fmt_txt_glyph_dsc_t *)
                                              heap_caps_malloc(loca_count * sizeof(lv_font_fmt_txt_glyph_dsc_t), MALLOC_CAP_SPIRAM);
...
static bool lvgl_load_font(lv_fs_file_t * fp, lv_font_t * font)
{
    lv_font_fmt_txt_dsc_t * font_dsc = (lv_font_fmt_txt_dsc_t *)
                                       heap_caps_malloc(sizeof(lv_font_fmt_txt_dsc_t), MALLOC_CAP_SPIRAM);
    ...
    bool failed = false;
    uint32_t * glyph_offset = heap_caps_malloc(sizeof(uint32_t) * (loca_count + 1), MALLOC_CAP_SPIRAM);
...
int32_t load_kern(lv_fs_file_t * fp, lv_font_fmt_txt_dsc_t * font_dsc, uint8_t format, uint32_t start)
{
    ...
    if(0 == kern_format_type) { /*sorted pairs*/
        lv_font_fmt_txt_kern_pair_t * kern_pair = heap_caps_malloc(sizeof(lv_font_fmt_txt_kern_pair_t), MALLOC_CAP_SPIRAM);
        ...
        uint8_t * glyph_ids = heap_caps_malloc(ids_size, MALLOC_CAP_SPIRAM);
        int8_t * values = heap_caps_malloc(glyph_entries, MALLOC_CAP_SPIRAM);
        ...
        lv_font_fmt_txt_kern_classes_t * kern_classes = heap_caps_malloc(sizeof(lv_font_fmt_txt_kern_classes_t), MALLOC_CAP_SPIRAM);
        ...
        uint8_t * kern_left = heap_caps_malloc(kern_class_mapping_length, MALLOC_CAP_SPIRAM);
        uint8_t * kern_right = heap_caps_malloc(kern_class_mapping_length, MALLOC_CAP_SPIRAM);
        int8_t * kern_values = heap_caps_malloc(kern_values_length, MALLOC_CAP_SPIRAM);
